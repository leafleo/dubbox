 ...
 */
@Activate
@Help(parameter = "[service.]method(args) [cxt_attachment<args>]", summary = "Invoke the service method.", detail = "Invoke the service method.")
    ...
    public String telnet(Channel channel, String message) {
        if (message == null || message.length() == 0) {
            return "Please input method name, eg: \r\ninvoke xxxMethod(1234, \"abcd\", {\"prop\" : \"value\"})\r\ninvoke XxxService.xxxMethod(1234, \"abcd\", {\"prop\" : \"value\"})\r\n"
            		...
            		+ "invoke com.xxx.XxxService.xxxMethod(1234, \"abcd\", {\"prop\" : \"value\"}) cxt_attachment<{\"prop\" : \"value\"}>";
        }
        ...
        if (i < 0 || ! message.contains(")")) {
            return "Invalid parameters, format: service.method(args)";
        }
        String method = message.substring(0, i).trim();
        String args = message.substring(i + 1, message.lastIndexOf(")")).trim();
        ...
        if (invoker != null) {
            if (invokeMethod != null) {
                try {
                    ...
                    //包括上下文信息
                    if(message.contains("cxt_attachment")) {
                    	int j = message.indexOf("<");
                        if (j < 0 || ! message.endsWith(">")) {
                            return "Invalid parameters, format: service.method(args) cxt_attachment<args>";
                        }
                        String args2 = message.substring(message.lastIndexOf("cxt_attachment<") + 15, message.length() - 1).trim();
                        Map<String, String> attachments = new HashMap<String, String>();
                        try {
                        	attachments = (Map<String, String>) JSON.parse( args2 , Map.class);
                        	cxt.getAttachments().putAll(attachments);
                        } catch (Throwable t) {
                            return "cxt_attachment<args> Invalid json argument, cause: " + t.getMessage();
                        } 
                    }
